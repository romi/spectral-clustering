...
begin = time.time()
model_filename = '/Users/katiamirande/PycharmProjects/Spectral_clustering_0/Data/chenos/cheno_C_2021_04_07.ply'
filenamemod = os.path.splitext(os.path.basename(model_filename))[0]
pcd = open3d.io.read_point_cloud(model_filename, format='ply')
r = 18
...
resegment_nodes_with_elbow_method(QG, QG_nodes_to_rework = list_of_linear, number_of_cluster_tested = 20, attribute='direction_gradient', number_attribute=3, standardization=False, numer = 1, G_mod = True, export_div=False)
...
#for n in list_leaves3 or n in list_apex:
...
delete_small_clusters(QG, min_number_of_element_in_a_quotient_node=10)
...
export_quotient_graph_attribute_on_point_cloud(QG, attribute='viterbi_class', name='semantic_tige_line_indif')
...
display_and_export_quotient_graph_matplotlib(quotient_graph=QG, node_sizes=20, filename=filenamemod+"_semantic_1_delete_small_clusters", data_on_nodes='viterbi_class', data=True, attributekmeans4clusters = False)

QG = merge_one_class_QG_nodes(QG, attribute='viterbi_class', viterbiclass=[5])
QG = merge_one_class_QG_nodes(QG, attribute='viterbi_class', viterbiclass=[1])
display_and_export_quotient_graph_matplotlib(quotient_graph=QG, node_sizes=20, filename=filenamemod+"_semantic_1_delete_merge", data_on_nodes='viterbi_class', data=True, attributekmeans4clusters = False)
...
def treat_topology_error2(QG, attribute_class_control='viterbi_class',number_attribute=1, class_limb = 1, class_linear = 0, error = 10, way_to_treat='norm_gradient', number_of_cluster_tested=20):
    ...
    for node_QG in list_nodes_QG_to_work:
        ...
        clusters_centers = resegment_nodes_with_elbow_method(QG, QG_nodes_to_rework=[node_QG], number_of_cluster_tested=number_of_cluster_tested,
                                          attribute=way_to_treat, number_attribute=number_attribute, standardization=False, numer=1, ret = True)
...
list_stem = select_all_quotientgraph_nodes_from_pointcloudgraph_cluster(G, QG, 3.0, attribute='viterbi_class')
for n in QG.nodes:
    if n in list_leaves3:
        QG.nodes[n]["viterbi_class"] = 1
